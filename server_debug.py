    ...
    try:
        ...
        for t in top_swaps:
            ...
            if fr and (fr < -0.0005 or fr > 0.0005):
                categories[sym] = "æ¥µç«¯è²»ç? (Squeeze Watch)"
                selected_symbols.append(sym)
                continue
            ...
            if pct > btc_pct + 5: # Outperforming BTC by 5%
                categories[sym] = "Alpha å¼·å‹¢å¹?(Rel. Strength)"
                selected_symbols.append(sym)
                continue
                
            # Deep Oversold
            if pct < -5:
                categories[sym] = "æ·±è?è¶…è³£ (Deep Oversold)"
                selected_symbols.append(sym)
                continue
            ...
            if low > 0 and ((high - low) / low) > 0.10:
                categories[sym] = "é«˜æ³¢?•ç???(High Volatility)"
                selected_symbols.append(sym)
                continue
        ...
        for t in top_swaps:
            ...
            if sym not in selected_symbols:
                categories[sym] = "?®é€šé???(High Volume)"
                selected_symbols.append(sym)
            ...
            for symbol in symbols:
                try:
                    ...
                    radar_item = {
                        ...
                        'pattern': 'None',
                        'score': 0,
                        'trigger_reason': '?ƒæ??‹æ?ä¸?(Scanning)',
                    ...
                    if patterns:
                        ...
                        if in_prz:
                            ...
                            radar_item['trigger_reason'] = 'PRZ ?µé?ï¼ç?å¾…æ?æ¨™å…±?¯é€²å ´'
                        else:
                            dist = min(abs(d_price - best_p.prz_low), abs(d_price - best_p.prz_high))
                            radar_item['score'] = max(10, int(100 - (dist / d_price) * 1000))
                            direction = "?šå?" if best_p.direction == 'bullish' else "?šç©º"
                            radar_item['trigger_reason'] = f"ç­‰å? D é»ž{direction}??{best_p.prz_center:.4g}"
                    ...
                    for p in patterns:
                        ...
                        if not sl_valid or not tp_valid:
                            block_reason = "API ?ƒæ•¸?¡æ? (æ­¢æ?/æ­¢ç??¹å??¯èª¤)"
                        elif not in_prz:
                            block_reason = "?ªé€²å…¥ PRZ ?€??
                        else:
                            # Apply Strategy-Specific Logic
                            t_dir = 'ç©? if htf_bear and not htf_bull else ('å¤? if htf_bull and not htf_bear else '??)
                            
                            if strategy_name == 'MacroSniper':
                                # Strict Patient Killer: Requires Trend + Institutional
                                if not trend_ok:
                                    block_reason = f"å¤§ç??¥è¶¨?¢ä?ç¬?({trend_tf} {t_dir})"
                                elif not div_ok and not sweep_ok:
                                    block_reason = "ç¼ºä?æ©Ÿæ?ä¿¡è? (?¡è????¡æ???"
                        ...
                        if not block_reason:
                            ...
                            if profit_pct < active_min_profit:
                                block_reason = f"?©æ½¤ç©ºé?å¤ªå? (<{(active_min_profit*100):.2f}%)"
                            elif true_rr < target_rr:
                                block_reason = f"?Ÿå¯¦?ˆè™§æ¯”é?ä½?({round(true_rr, 2)} < {target_rr})"
                        ...
                        if not block_reason and not already_active and not on_cooldown:
                            ...
                            try:
                                ...
                                signal_obj["entry_reason"] = f"è§¸ç™¼ {p.pattern_name} (?‡æ??±æŒ¯/è¶¨å‹¢?»å?)"
            ...
            try:
                ...
                for t in active_trades:
                    if t['status'] == 'active':
                        ...
                        # ?? DYNAMIC TRAILING STOP MECHANISM (?•æ?è¿½è¹¤æ­¢ç?)
                                    ...
                                    if is_better_sl and sl_diff_pct > 0.0005:
                                        ...
                                        if (t['direction'] == 'long' and new_sl >= t['current']) or \
                                           (t['direction'] == 'short' and new_sl <= t['current']):
                                            print(f"[?š¨ EMERGENCY] {t['symbol']} trailing SL {new_sl} already breached by market {t['current']}. Closing immediately!")
                                            try:
                                                ...
                                                if pos and pos.get('contracts') and float(pos['contracts']) > 0:
                                                    ...
                                                    print(f"[?š¨ EMERGENCY] {t['symbol']} closed successfully to prevent OKX Error 51278.")
                                                else:
                                                    print(f"[?š¨ EMERGENCY] {t['symbol']} position not found or already closed.")
                                            except Exception as e:
                                        ...
                                        for a in matching_algos:
                                            try:
                                                ...
                                                if res.get('code') == '0':
                                                    print(f"[?›¡ï¸?TRAILING STOP] {t['symbol']} SL moved to: {formatted_sl} (Progress: {highest_progress*100:.1f}%)")
                                                    t['sl'] = new_sl
                                                else:
...
def api_intelligence():
    ...
    return jsonify({
        'logic': 'æ¯?60 ?†é??ªå???OKX äº¤æ??€è«‹æ???100 å¤§æ??•æ€§å?ç´„ï?ä¸¦é€é?é«˜æ??¨å®¶æ¼”ç?æ³?è³‡é?è²»ç??°å¸¸?ç›¸å°å¤§?¤å¼·å¼±ã€æ¥µç«¯æ³¢??ç¯©é¸?ºæ??©å?è«§æ³¢äº¤æ???30 å¤§é??›é™£å®¹ã€?,
